...
import { Plus, Search, Pencil, Trash2, Loader2, Info } from 'lucide-react';
...
const AdminAbout: React.FC = () => {
  ...
  const [selected, setSelected] = useState<AboutValue | null>(null);
  const { toast } = useToast();

  const fetchItems = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('about_values').select('*').order('display_order');
  ...
  const handleSubmit = async (form: any) => {
    const payload = { title: form.title, description: form.description, icon: form.icon, display_order: form.display_order, is_active: form.is_active };
    if (selected) {
      const { error } = await supabase.from('about_values').update(payload).eq('id', selected.id);
      ...
      if (error) throw error;
      toast({ title: 'Created', description: 'Value created successfully' });
    }
    setSelected(null);
    fetchItems();
  };
  ...
  const handleDelete = async () => {
    if (!selected) return;
    const { error } = await supabase.from('about_values').delete().eq('id', selected.id);
    if (error) toast({ title: 'Error', description: error.message, variant: 'destructive' });
    else { toast({ title: 'Deleted', description: 'Value deleted' }); fetchItems(); }
    setSelected(null);
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3"><Info className="w-8 h-8 text-[#00C2D1]" /><h1 className="text-2xl font-bold text-gray-900">About - Company Values</h1></div>
        <Button onClick={() => { setSelected(null); setModalOpen(true); }} className="bg-[#003B8E]"><Plus className="w-4 h-4 mr-2" />Add Value</Button>
      </div>
      ...
      <ContentFormModal isOpen={modalOpen} onClose={() => setModalOpen(false)} onSubmit={handleSubmit} initialData={selected} title="Company Value" showIcon showImage imageFolder="about" />
      <DeleteConfirmDialog isOpen={deleteOpen} onClose={() => setDeleteOpen(false)} onConfirm={handleDelete} title="Delete Value" description={`Delete "${selected?.title}"?`} />
    </div>
  );
};
