...
const AdminPlatforms: React.FC = () => {
  ...
  const handleSubmit = async (form: any) => {
    ...
    if (selected) {
      const { error } = await supabase.from('platforms').update(payload).eq('id', selected.id);
      if (error) throw error;
      toast({ title: 'Updated', description: 'Platform updated successfully' });
    } else {
      const { error } = await supabase.from('platforms').insert([payload]);
      if (error) throw error;
      toast({ title: 'Created', description: 'Platform created successfully' });
    }
    setSelected(null);
    fetchItems();
  };
  ...
  const handleDelete = async () => {
    if (!selected) return;
    const { error } = await supabase.from('platforms').delete().eq('id', selected.id);
    if (error) toast({ title: 'Error', description: error.message, variant: 'destructive' });
    else { toast({ title: 'Deleted', description: 'Platform deleted' }); fetchItems(); }
    setSelected(null);
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={() => { setSelected(null); setModalOpen(true); }} className="bg-[#003B8E]"><Plus className="w-4 h-4 mr-2" />Add Platform</Button>
      </div>
      ...
      <ContentFormModal isOpen={modalOpen} onClose={() => setModalOpen(false)} onSubmit={handleSubmit} initialData={selected} title="Platform" showIcon showImage imageFolder="platforms" />
      <DeleteConfirmDialog isOpen={deleteOpen} onClose={() => setDeleteOpen(false)} onConfirm={handleDelete} title="Delete Platform" description={`Delete "${selected?.title}"?`} />
    </div>
  );
};
